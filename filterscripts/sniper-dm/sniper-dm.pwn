...
public OnEventInit() {
    ...
    SendClientMessageToAll(0x00FF00FF, "** Event Sniper Deathmatch has begun! Use /sniper to join!");
    return 1;
}

public OnEventExit() {
    
    SendClientMessageToAll(0x00FF00FF, "** Event Sniper Deathmatch has ended!");
    RemoveAllPlayers();
    return 1;
}

public OnPlayerJoinEvent(playerid) {
    SendClientMessage(playerid, 0x00FF00FF, "Welcome! Your objective is to kill every other player with your sniper!");
...
public OnPlayerLeaveEvent(playerid) {
    // remove their sniper and respawn them
    ResetPlayerWeapons(playerid);
